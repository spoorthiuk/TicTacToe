...
class MainActivity:AppCompatActivity() {
    /**
     * This class defines the main Human VS AI gameplay - Implemented Minimax with Alpha Beta pruning (Hard),
     * Random Selection (Easy) and Suboptimal Selection (Medium)
     */
    ...
    private fun getWinCombination(player: String): Array<Array<Int>>?
    {
        ...
        for (i in 0..2) {
            if (gridBoxes[i][0].tag == player && gridBoxes[i][1].tag == player && gridBoxes[i][2].tag == player) {
                Toast.makeText(this@MainActivity, "You $state at ${i+1}th Row", Toast.LENGTH_SHORT).show()
        ...
        for (i in 0..2) {
            if (gridBoxes[0][i].tag == player && gridBoxes[1][i].tag == player && gridBoxes[2][i].tag == player) {
                Toast.makeText(this@MainActivity, "You $state at ${i+1}th Column", Toast.LENGTH_SHORT).show()
        ...
        if (gridBoxes[0][0].tag == player && gridBoxes[1][1].tag == player && gridBoxes[2][2].tag == player) {
            Toast.makeText(this@MainActivity, "You $state at top left to bottom right diagonal", Toast.LENGTH_SHORT).show()
        ...
        if (gridBoxes[0][2].tag == player && gridBoxes[1][1].tag == player && gridBoxes[2][0].tag == player) {
            Toast.makeText(this@MainActivity, "You $state at bottom left to top right diagonal", Toast.LENGTH_SHORT).show()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MainActivity, "Game result saved!", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                // Handle any errors
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@MainActivity, "Error saving result: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
